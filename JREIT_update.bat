...
set "JREIT_SELF=%~f0"
set "JREIT_TARGET=%~1"
powershell.exe -NoProfile -STA -Command "$ErrorActionPreference='Stop'; try { $s=[IO.File]::ReadAllText($env:JREIT_SELF,[Text.Encoding]::UTF8); $a=[regex]::Split($s,'(?m)^# JREIT_POWERSHELL\r?$'); if($a.Length -ne 2){throw 'Script marker missing.'}; & ([scriptblock]::Create($a[1])) } catch { Write-Host ('ERROR: '+$_.Exception.Message); exit 1 }"
...
function Read-Answer([string]$text) {
    ...
    $text = [regex]::Replace($text,'\A```(?:json)?\s*','','IgnoreCase')
...
function Convert-Record($row, [double]$fallback) {
    ...
    foreach ($p in $row.PSObject.Properties) {
        ...
        if ($fields.ContainsKey($field)) {
            ...
            elseif ($field -in $moneyFields -or $field -in @('area','rentable')) { $equal = ([math]::Abs($prior-$v) -le 1e-10 * [math]::Max(1,[math]::Abs($v))) }
    ...
    if ($fields.transaction -eq '取得' -and $fields.ContainsKey('price') -and $fields.ContainsKey('acquisition') -and [math]::Abs($fields.price-$fields.acquisition) -gt 1e-10 * [math]::Max(1,[math]::Abs($fields.acquisition))) { throw 'The transaction price and acquisition price disagree for an acquisition. Check the JSON units and source.' }
...
'@
    ...
    foreach ($write in $writes) {
        ...
        foreach ($format in $write.Formats) {
            ...
            try { $cell.NumberFormat = [string]$format[1] } finally { Release-Com $cell }
        }
    }
    ...
    for ($r=0; $r -lt $count; $r++) {
        ...
        if ($f.ContainsKey('noi') -and $f.ContainsKey('acquisition') -and $f.acquisition -ne 0) {
            ...
            if ($null -eq $actual -or $actual -is [string] -or [math]::Abs(([double]$actual)-$expected) -gt 1e-10 * [math]::Max(1,[math]::Abs($expected))) { throw 'NOI yield validation failed. The workbook was not saved.' }
